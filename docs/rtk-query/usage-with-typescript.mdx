...
- Typescript와 함께 RTK Query의 여러 API를 사용하는 방법

:::

## Introduction

Redux Toolkit 패키지의 타 부분과 마찬가지로 RTK Query는 TypeScript로 작성되었으며 해당 API는 TypeScript 애플리케이션에서 원활하게 사용하도록 설계되었습니다.

이 페이지에서는 TypeScript와 함께 RTK Query에 포함된 API를 사용하고 올바르게 입력하는 방법에 대한 세부 정보를 제공합니다.

:::info

**최고의 결과를 위해 RTK Query와 함께 Typescript 4.1+ 을 사용하는 것을 권장합니다.**
...
TypeScript 사용자가 자동 생성된 React Hooks를 사용하기 위해서는 **TS4.1+ 의 버전이 필요합니다**.
...
이전 버전의 TS를 사용하고 있다면, `api.endpoints.[endpointName].useQuery/useMutation` 를 통해 동일한 hooks에 접근할 수 있습니다.
...
`BaseQueryFn` 타입은 다음의 generic들을 가집니다:

- `Args` - 함수의 첫번째 파라미터 타입입니다. The result returned by a [`query`](./api/createApi.mdx#query) property on an endpoint will be passed here.
- `Result` - success case의 경우에 반환될 `data` 속성의 type 입니다. Unless you expect all queries and mutations to return the same type, it is recommended to keep this typed as `unknown`, and specify the types individually as shown [below](#typing-query-and-mutation-endpoints).
- `Error` - error case의 경우에 반환될 `error` 속성의 type 입니다. This type also applies to all [`queryFn`](#typing-a-queryfn) functions used in endpoints throughout the API definition.
