...
hide_title: true
description: 'RTK Query > 사용법 > Migrating to RTKQ: how to convert logic to RTKQ'
---
...
- 기존의 Redux Toolkit + `createAsyncThunk` 로 구현된 data-fetching logic을 Redux Toolkit Query를 사용하도록 변환하는 방법
...
RTK Query는 data fetching을 위해 특별히 제작되었습니다. While it can't replace all of the situations where you'd use thunks or other side effects approaches, **using RTK Query should eliminate the need for most of that hand-written side effects logic**.
...
아래의 정의한 selectors를 통해, 주어진 pokemon name에 대한 status & data에 접근이 가능하도록 합니다.
...
자동으로 생성된 hook의 사용법은 위의 custom hook과 동일합니다! 해야 할 일은 단지 import 경로를 변경하기만 하면 됩니다!
...
이전에 언급했듯이, 작성한 `api` 정의는 기존의 `createAsyncThunk`, `createSlice` 및 커스텀 hook의 기능을 대체합니다.

더 이상 해당 슬라이스를 사용하지 않으므로, store에서 import 및 reducer를 제거할 수 있습니다:
...
또한 _모든 slice와 hook 파일_ 을 완전히 삭제할 수 있습니다!
...
이제 20줄 미만의 코드로 전체 디자인 사양(및 그 이상!)을 다시 구현했으며, api 정의에 엔드포인트를 추가하여 쉽게 확장할 수 있습니다.

RTK Query를 사용하여 리팩토링된 구현의 실행 가능한 예는 아래에서 볼 수 있습니다:
