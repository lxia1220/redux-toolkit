...
hide_title: true
description: 'RTK Query > 사용법 > Queries: fetching data from a server'
---
...
React 전용 버전의 `createApi`에서 자동적으로 생성된 React hook는 주어진 쿼리의 현재 상태를 반영하는 [derived booleans](#frequently-used-query-hook-return-values)을 제공합니다. Derived booleans are preferred for the generated React hooks as opposed to a `status` flag, as the derived booleans are able to provide a greater amount of detail which would not be possible with a single `status` flag, as multiple statuses may be true at a given time (such as `isFetching` and `isSuccess`).

쿼리 엔드포인트에서 RTK Query는 좀 더 유연하게 정보들을 제공하기 위해 `isLoading`과 `isFetching`을 구분하고 있습니다.

- `isLoading` 은 _처음으로_ 쿼리가 실행중인지를 나타냅니다. 이 시간동안에는 데이터를 사용할 수 없습니다.
- `isFetching` 은 주어진 엔드포인트 + 쿼리 매개변수로 실행중인 쿼리를 의미하지만, 반드시 처음으로 실행중인 쿼리를 나타내는 것은 아닙니다. Data may be available from an earlier request done by this hook, maybe with the previous query param.

이러한 구분을 통해서 UI에 더 많은 제어권을 가질 수 있습니다. 예를 들어 `isLoading`을 통해 처음 로딩하는 동안에 스켈레톤을 보여줄 수 있고, `isFetching`을 통해 1페이지에서 2페이지로 페이지를 변경하거나 데이터를 무효화하여 다시 가져오고 있을 경우 이전 데이터를 회색으로 보여줄 수 있습니다.
...
Query를 수행할 때 RTK Query는 요청 파라미터를 자동으로 직렬화하고 요청에 대한 `queryCacheKey` 를 내부적으로 생성합니다. Any future request that produces the same `queryCacheKey` will be de-duped against the original, and will share updates if a `refetch` is trigged on the query from any subscribed component. 이후 동일한 `queryCacheKey` 를 가지는 모든 요청들은 원본 데이터에 의해서 중복 제거되고, `refetch`가 발생했을 경우 이를 구독한 컴포넌트간 업데이트를 공유합니다.
