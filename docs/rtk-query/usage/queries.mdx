...
이 예는 중복 요청 제거 및 캐싱 동작을 보여줍니다:

1. 첫 'bulbasaur' 유형의 `Pokemon` component 가 추가되고, 'bulbasaur' 데이터를 요청을 통해 가져옵니다.
2. 그 뒤, 다른 'bulbasaur' 유형의 `Pokemon` component가 추가 됩니다.
   - 이 경우 'Loading...' 이 표시되지 않고 네트워크 요청이 발생하지 않는 것을 볼 수 있나요? 여기서 캐시가 사용됩니다.
3. 잠시 후, 'pikachu' 유형의 `Pokemon` component 가 추가되고, 'pikachu' 데이터를 요청을 통해 가져옵니다.
4. 특정 포켓몬 유형의 'Refetch' 를 누를 경우, 한번의 요청으로 해당 포켓몬 유형의 모든 component가 업데이트 됩니다.
