...
hide_title: true
description: 'RTK Query > 사용법 > Pagination: suggestions for handling paginated data'
---
...
RTK Query에는 Pagination 기능이 포함되어 있지 않습니다. 그러나 RTK Query를 통해 표준 인덱스 기반 pagination API와 쉽게 통합할 수 있습니다. 이것은 구현해야 하는 가장 일반적인 pagination 형식입니다.

## Pagination Recipes

### Endpoint에서 arg로 page를 받도록 설정합니다.
...
### `page` state 변수를 증가시켜 다음 페이지를 트리거합니다.
