...
이 미들웨어는 [`configureStore`](./configureStore.mdx) 의 [`getDefaultMiddleware`](./getDefaultMiddleware.mdx) 에 의해 기본적으로 store에 추가됩니다.

`getDefaultMiddleware` 에 전달되는 옵션 중 `immutableCheck` 값을 통해 이 미들웨어의 동작을 사용자 정의할 수 있습니다.
...
주어진 옵션으로 immutability check middleware의 인스턴스를 생성합니다.
